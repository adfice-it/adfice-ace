...
async function main() {
    ...
    try {
		...
		var medCounter = 0;
		do {
			var atcCode = await meds[medCounter].ATC_code;
			medsWithRulesToFire[atcCode] = medsWithRulesToFire[atcCode] || [];
			var rulesToFire = await determineWhichRulesApply(atcCode, rules);
        	medsWithRulesToFire[atcCode].push(rulesToFire);
			medCounter++;
        ...
        if (conn) {
            let result = conn.end();
            pool.end();
console.log(util.inspect(medsWithRulesToFire['C03AA03']));
console.log(util.inspect(medsWithRulesToFire['C09AA02']));
            return result;
        }
    }
}

function determineWhichRulesApply (atcCode, rules){
